...
import br.com.entra21.backend.spring.projeto.repository.IProgramadorReposytory;
...
public class ProgramadorController {
	...
	@Autowired
	private IProgramadorReposytory programadorRepository;
	...
	public @ResponseBody Optional<Programador> update(@PathVariable("id") int id,
		...
		atual.setQtd_linguagem(programadorEditado.getQtd_linguagem());
	...
	// programador.getLinks().add(new ItemNivel3("GET", PATH + "/" +
	// programador.getId(), null, null));

	// programador.getLinks().add(new ItemNivel3("POST", PATH, headers,
	// jsonCreate));

	// programador.getLinks().add(new ItemNivel3("PUT", PATH + "/" +
